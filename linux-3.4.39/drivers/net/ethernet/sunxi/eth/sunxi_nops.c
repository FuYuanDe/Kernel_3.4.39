...
typedef union {
	struct {
		/* TDES0 */
		volatile unsigned int deferred:1;	/* Deferred bit (only half-duplex) */
		volatile unsigned int under_err:1;	/* Underflow error */
		volatile unsigned int ex_deferral:1;	/* Excessive deferral */
		volatile unsigned int coll_cnt:4;	/* Collision count */
		volatile unsigned int vlan_tag:1;	/* VLAN Frame */
		volatile unsigned int ex_coll:1;		/* Excessive collision */
		volatile unsigned int late_coll:1;	/* Late collision */
		volatile unsigned int no_carr:1;		/* No carrier */
		volatile unsigned int loss_carr:1;	/* Loss of collision */
		volatile unsigned int ipdat_err:1;	/* IP payload error */
		volatile unsigned int frm_flu:1;		/* Frame flushed */
		volatile unsigned int jab_timeout:1;	/* Jabber timeout */
		volatile unsigned int err_sum:1;		/* Error summary */
		volatile unsigned int iphead_err:1;	/* IP header error */
		volatile unsigned int ttss:1;		/* Transmit time stamp status */
		volatile unsigned int reserved0:13;
		volatile unsigned int own:1;		/* Own bit. CPU:0, DMA:1 */
	} tx;
	...
	struct {
		/* RDES0 */
		volatile unsigned int chsum_err:1;	/* Payload checksum error */
		volatile unsigned int crc_err:1;		/* CRC error */
		volatile unsigned int dribbling:1;	/* Dribble bit error */
		volatile unsigned int mii_err:1;		/* Received error (bit3) */
		volatile unsigned int recv_wt:1;		/* Received watchdog timeout */
		volatile unsigned int frm_type:1;	/* Frame type */
		volatile unsigned int late_coll:1;	/* Late Collision */
		volatile unsigned int ipch_err:1;	/* IPv header checksum error (bit7) */
		volatile unsigned int last_desc:1;	/* Laset descriptor */
		volatile unsigned int first_desc:1;	/* First descriptor */
		volatile unsigned int vlan_tag:1;	/* VLAN Tag */
		volatile unsigned int over_err:1;	/* Overflow error (bit11) */
		volatile unsigned int len_err:1;		/* Length error */
		volatile unsigned int sou_filter:1;	/* Source address filter fail */
		volatile unsigned int desc_err:1;	/* Descriptor error */
		volatile unsigned int err_sum:1;		/* Error summary (bit15) */
		volatile unsigned int frm_len:14;	/* Frame length */
		volatile unsigned int des_filter:1;	/* Destination address filter fail */
		volatile unsigned int own:1;		/* Own bit. CPU:0, DMA:1 */
	#define RX_PKT_OK		0x7FFFB77C
	#define RX_LEN			0x3FFF0000
	} rx;
...
typedef union {
	struct {
		/* TDES1 */
		volatile unsigned int buf1_size:11;	/* Transmit buffer1 size */
		volatile unsigned int buf2_size:11;	/* Transmit buffer2 size */
		volatile unsigned int ttse:1;		/* Transmit time stamp enable */
		volatile unsigned int dis_pad:1;		/* Disable pad (bit23) */
		volatile unsigned int adr_chain:1;	/* Second address chained */
		volatile unsigned int end_ring:1;	/* Transmit end of ring */
		volatile unsigned int crc_dis:1;		/* Disable CRC */
		volatile unsigned int cic:2;		/* Checksum insertion control (bit27:28) */
		volatile unsigned int first_sg:1;	/* First Segment */
		volatile unsigned int last_seg:1;	/* Last Segment */
		volatile unsigned int interrupt:1;	/* Interrupt on completion */
	} tx;

	struct {
		/* RDES1 */
		volatile unsigned int buf1_size:11;	/* Received buffer1 size */
		volatile unsigned int buf2_size:11;	/* Received buffer2 size */
		volatile unsigned int reserved1:2;
		volatile unsigned int adr_chain:1;	/* Second address chained */
		volatile unsigned int end_ring:1;		/* Received end of ring */
		volatile unsigned int reserved2:5;
		volatile unsigned int dis_ic:1;		/* Disable interrupt on completion */
	} rx;
...
///-
#ifndef USE_PHY_8201
    value = readl(iobase + GETH_BASIC_CTL0);
    value |= 0x01;

    /* Default is 1000Mbps */
    value &= ~0xc;
    writel(value, iobase + GETH_BASIC_CTL0);
#endif
