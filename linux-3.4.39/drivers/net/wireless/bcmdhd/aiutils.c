...
///-
    ...
    {
        int test_i;
        uint32 word, *ptr = 0x0a04c;
        for(test_i=0; test_i < 100; test_i++)
        {
            //sdioh_request_word(((bcmsdh_info_t *)osl_get_bus_handle(sii->osh))->sdioh, SDIOH_CMD_TYPE_NORMAL,
            //        SDIOH_READ, SDIO_FUNC_1, 0x0a04c, &word, 4);
            R_REG(sii->osh, &cc->eromptr);
            R_REG(sii->osh, ptr);
        }
    }
