...
#define FAILSAFE_FILE   "/tmp/failsafe_button"
...
static struct timer_list key_check_timer;
static int check_cnt = 0;
static uint32_t gpio_bit_map = 0, gpio_bit_mask = 0x3;
static struct gpio_keys_button_data *global_bdata[2] = {NULL};
...
static void key_check(unsigned long data)
{
///-
#if 0
    int value;
    ...
    check_cnt++;
    if(!gpio_button_get_value(bdata))
    {
        check_cnt = 0;
        return;
    }
    if(check_cnt >= 10)
    {
        ...
        pfile = filp_open(FAILSAFE_FILE,  O_WRONLY | O_CREAT, 0755);
        if(pfile)
        {
            set_fs(get_ds());
            btn = button_get_index(bdata->b->code);
            vfs_write(pfile, button_map[btn].name, strlen(button_map[btn].name), &tmpoffset);
            set_fs(old_fs);
    ...
    {
        key_check_timer.expires = jiffies + (HZ*20)/1000;
        add_timer(&key_check_timer);
    }
#else
    int btn0, btn1;
    char buf[128];
    ...
    check_cnt++;
    if((!gpio_button_get_value(global_bdata[0])) || (!gpio_button_get_value(global_bdata[1])))
    {
        check_cnt = 0;
        return;
    }
    if(check_cnt >= 10)
    {
        ...
        {
            set_fs(get_ds());
            btn0 = button_get_index(global_bdata[0]->b->code);
            btn1 = button_get_index(global_bdata[1]->b->code);
            memset(buf, 0, sizeof(buf));
            snprintf(buf, sizeof(buf), "%s %s", button_map[btn0].name, button_map[btn1].name);
            vfs_write(pfile, buf, strlen(buf), &tmpoffset);
            set_fs(old_fs);
            filp_close(pfile, NULL);
            pfile = NULL;
        }
...
///-
        /*系统启动过程中按下键的事件，由于模块没有加载，irq不会触发，这里主动创建文件，
          不能使用发送事件的方式，hotplug_call收不到。
          进入failsafe模式需要用到*/

        btn = button_get_index(bdata->b->code);
///-
#if 0
        if(0 == btn && gpio_button_get_value(bdata))
        {
            init_timer(&key_check_timer);
            key_check_timer.expires = jiffies + (HZ*20)/1000;
            key_check_timer.data = (unsigned long)bdata;
            key_check_timer.function = key_check;   /* timer handler */
            add_timer(&key_check_timer);
        }
#else
        if((0 == btn) || (1 == btn))
        {
            gpio_bit_map |= 1 << btn;
            global_bdata[btn] = bdata;
            if(((gpio_bit_map & gpio_bit_mask) == gpio_bit_mask) &&
                gpio_button_get_value(global_bdata[0]) &&
                gpio_button_get_value(global_bdata[1]))
            {
                init_timer(&key_check_timer);
                key_check_timer.expires = jiffies + (HZ*20)/1000;
                key_check_timer.data = (unsigned long)NULL;
                key_check_timer.function = key_check;   /* timer handler */
                add_timer(&key_check_timer);
            }
        }
...
static struct gpio_keys_button sunxi_gpio_keys_button[] =
{
    {
        .code = 0x100,
        .gpio = 224,
...
static struct gpio_keys_platform_data sunxi_gpio_keys_platform_data =
{
    .buttons = sunxi_gpio_keys_button,
    .nbuttons = sizeof(sunxi_gpio_keys_button)/sizeof(sunxi_gpio_keys_button[0]),
    .poll_interval = 20,
};

static void sunxi_gpio_release (struct device *dev)
{
...
static struct platform_device gpio_keys_device =
{
    ...
    .id = -1,
    .dev = {
        .platform_data = (void *)&sunxi_gpio_keys_platform_data,
