...
///-
struct mutex pin_share_mutex;
EXPORT_SYMBOL(pin_share_mutex);

static int sunxi_register_spidev_by_master(struct spi_master *master);
...
static void sunxi_spi_gpio_cfg(void)
{
    uint32_t reg_val;
    
    writel(0x3333, PC_CFG0_REG);
	reg_val = readl(PC_PULL0_REG);	  // PIO SETTING,PortC SPI0 pull reg
	reg_val &= ~(0x03 << 4);
	reg_val |= (0x01 << 4); 
	writel(reg_val, PC_PULL0_REG);
}
 ...
 */
static int sunxi_spi_xfer_small(struct spi_device *spi, struct spi_transfer *t)
{
	struct sunxi_spi *sspi = spi_master_get_devdata(spi->master);
	void __iomem* base_addr = sspi->base_addr;
	unsigned long flags = 0;
        ...
        switch(sspi->mode_type)
        {
            case SINGLE_HALF_DUPLEX_RX:
            {
                while(rx_len > 0)
                {
                    ...
                    if(rx_len > BULK_DATA_BOUNDARY)
                    {
                        tmp_len = BULK_DATA_BOUNDARY;
                    }
                    else
                    {
                        tmp_len = rx_len;
                    ...
                    while(0)
                    {
                        ...
                        val = readl(base_addr + SPI_INT_STA_REG);
                        if(val & SPI_INTEN_RX_EMP)
                        {
                            writel(val & SPI_INTEN_RX_EMP, base_addr + SPI_INT_STA_REG);
                            break;
                        }
                    }
                }
            ...
            case SINGLE_HALF_DUPLEX_TX:
            {
                while(tx_len > 0)
                {
                    ...
                    while(0)
                    {
                        ...
                        val = readl(base_addr + SPI_INT_STA_REG);
                        if(val & SPI_INTEN_TX_EMP)
                        {
                            writel(val & SPI_INTEN_TX_EMP, base_addr + SPI_INT_STA_REG);
                            break;
                        }
                    }
                }
...
static int sunxi_spi_xfer(struct spi_device *spi, struct spi_transfer *t)
{
	struct sunxi_spi *sspi = spi_master_get_devdata(spi->master);
    int remain, done;
    void *save_tx_buf, *save_rx_buf;

    if ((!t->tx_buf && !t->rx_buf) || !t->len)
		return -EINVAL;

    save_tx_buf = t->tx_buf;
    save_rx_buf = t->rx_buf;
    remain = t->len;
    done = 0;
    while(remain > 0)
    {
        if(remain > BULK_DATA_BOUNDARY)
        {
            t->len = BULK_DATA_BOUNDARY;
        }
        else
        {
            t->len = remain;
        ...
        if(t->tx_buf)
        {
            t->tx_buf = (void *)(((char *)save_tx_buf) + done);
        }
        if(t->rx_buf)
        {
            t->rx_buf = (void *)(((char *)save_rx_buf) + done);
        }
        if(sunxi_spi_xfer_small(spi, t))
        {
            SPI_ERR("sunxi_spi_xfer error\n");
            return -1;
        }
        remain -= t->len;
        done += t->len;
    ...
    t->len = done;
    t->tx_buf = save_tx_buf;
    t->rx_buf = save_rx_buf;
...
///-
			...
			//status = sunxi_spi_xfer_small(spi, t);
			if (status)
...
///-
	...
	if (sspi->pctrl != NULL)
    {
        ret = sunxi_spi_select_gpio_state(sspi->pctrl, PINCTRL_STATE_DEFAULT, bus_no);
        mutex_unlock(&pin_share_mutex);
		return ret;
    }
...
///-
    sunxi_register_spidev_by_master(master);
...
static int sunxi_register_spidev_by_master(struct spi_master *master)
{
    ...
    int i, spidev_num;
    char spi_board_name[32] = {0};
    struct spi_board_info *board, board_tmp;
    ...
    for (i=0; i<spidev_num; i++) {
        memset(&board_tmp, 0, sizeof(board_tmp));
        board = &board_tmp;
        sprintf(spi_board_name, "spi_board%d", i);
        ...
        if(i >= 1)
        {
            ...
            if(board->bus_num != master->bus_num)
            {
                SPI_ERR("spi bus num %d != master bus num %d\n", board->bus_num, master->bus_num);
                //goto fail;
    			continue;
            }
            
    		sprintf(board->modalias, "%s", "spidev");
            board->max_speed_hz = 4000000;
            board->chip_select = i - 1;
            board->mode = 0;
        }
        else
#endif
