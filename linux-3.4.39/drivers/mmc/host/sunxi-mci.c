...
static int gpio_sd_switch = GPIOH(10);//PH10
#elif defined(PRODUCT_UC200)
static int gpio_sd_switch = GPIOH(1); //PH1
#endif
...
#if defined(PRODUCT_SBC300MAIN) || defined(PRODUCT_UC200) || defined(PRODUCT_SBC1000MAIN)
    /**由于板子上电会去检查sd卡，导致启动失败，所以默认下启动不把sd卡开关打开，
        在驱动加载sd卡开关再打开开关*/
    ret = gpio_request(gpio_sd_switch, "gpio_sd_switch");
    if(ret < 0)
    {
        SMC_MSG(NULL, "gpio_sd_switch request fail");
        return -1;
    }
    gpio_direction_output(gpio_sd_switch, 0);
#endif
