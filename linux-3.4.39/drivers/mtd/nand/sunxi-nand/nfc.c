...
static dma_addr_t read_buffer_dma = 0;
static dma_addr_t write_buffer_dma = 0;
static int dma_hdle;
...
static int nand_irq_request = 0;

///-
extern struct mutex pin_share_mutex;
...
#else
static int sunxi_nand_clkrequest(uint32_t nand_index)
{
    uint32_t cfg;
    uint32_t m, n;
    uint32_t clk_src;
    ...
    if(nand_index == 0)
    {
        ...
        //reset
        cfg = readl(BUS_SOFT_RST_REG0);
        cfg &= (~(0x1<<13));
        writel(cfg, BUS_SOFT_RST_REG0);
        ...
        ERR_INFO("NAND_ClkRequest, nand_index: 0x%x\n", nand_index);
        ERR_INFO("Reg 0x01c20080: 0x%x\n", readl(NAND_SCLK_CFG_REG));
        ERR_INFO("Reg 0x01c20060: 0x%x\n", readl(AHB_GATING_REG0));
        ERR_INFO("Reg 0x01c202c0: 0x%x\n", readl(BUS_SOFT_RST_REG0));
    }    

    return 0;
}

static int sunxi_nand_setclk(uint32_t nand_index, uint32_t nand_clock)
{
	uint32_t edo_clk;
	uint32_t cfg;
	uint32_t m = 0, n = 0;
	...
	if(nand_index == 0)
	{
        ...
        writel(cfg, NAND_SCLK_CFG_REG);
		ERR_INFO("NAND_SetClk, nand_index: 0x%x\n", nand_index);
		ERR_INFO("Reg 0x01c20080: 0x%x\n", readl(NAND_SCLK_CFG_REG));
...
static void sunxi_set_nand_clock(uint32_t nand_max_clock)
{
    sunxi_nand_clkrequest(0);
    sunxi_nand_setclk(0, 10);
}
#endif
...
///-
	...
	writel(0x00000222, PC_CFG2_REG);
    cfg = readl(PC_CFG3_REG);
    cfg &= (~0xfff);
    cfg |= 0x222;
    writel(cfg, PC_CFG3_REG);
#endif
#else
    uint32_t cfg;
    ...
    writel(0x22222222, PC_CFG0_REG);
    writel(0x22222222, PC_CFG1_REG);
    cfg = readl(PC_CFG2_REG);
    cfg &= (~0xff);
    cfg |= 0x22;
    writel(cfg, PC_CFG2_REG);
    cfg = readl(PC_CFG3_REG);
    cfg &= (~0xfff);
    cfg |= 0x222;
    writel(cfg, PC_CFG3_REG);
...
static void sunxi_test_nand_pio(void)
{
    uint32_t cfg;

    cfg = readl(PC_CFG0_REG);
    if(0x22222222 != cfg)
    {
        printk(KERN_ERR "!!!!!!!!nand gpio modify by other!!!!!!!!!PC_CFG0_REG:%x\n", cfg);
        return;
    }
    cfg = readl(PC_CFG1_REG);
    if(0x22222222 != cfg)
    {
        printk(KERN_ERR "!!!!!!!!nand gpio modify by other!!!!!!!!!PC_CFG1_REG:%x\n", cfg);
        return;
    }
    cfg = readl(PC_CFG2_REG);
    if(0x22 != (cfg & 0xff))
    {
        printk(KERN_ERR "!!!!!!!!nand gpio modify by other!!!!!!!!!PC_CFG2_REG:%x\n", cfg);
        return;
    }
}
...
static void __nfc_cmdfunc(struct mtd_info *mtd, unsigned command, int column,
						int page_addr)
{
	int i;
	...
	case NAND_CMD_READ0:
		...
		dma_nand_config_start(dma_hdle, 0, (uint32_t)read_buffer_dma, read_size);
		addr_cycle = 5;
		// RAM0 is 1K size
		byte_count =1024;
		wait_rb_flag = 1;
...
///-
        /*dma从内存向设备搬移数据之前，如果dma内存是采用的dma_map_single函数获取的，
          系统不会保证cpu cache与内存的一致性，需要显示的调用dma_sync_single_for_device
          将cache写回到内存*/
        dma_sync_single_for_device(NULL, write_buffer_dma, buffer_size, DMA_TO_DEVICE);
...
///-
		...
		dma_nand_config_start(dma_hdle, 1, (uint32_t)write_buffer_dma, write_size);
		// RAM0 is 1K size
		byte_count =1024;
	...
	case NAND_CMD_RESET:
		...
		break;
    case NAND_CMD_READOOB:
        dma_sync_single_for_cpu(NULL, read_buffer_dma, buffer_size, DMA_FROM_DEVICE);
        break;
	case NAND_CMD_READ0:
///-
        /*dma从设备向内存搬移数据之前，如果dma内存是采用的dma_map_single函数获取的，
          系统不会保证cpu cache与内存的一致性，需要显示的调用dma_sync_single_for_cpu
          使cache无效*/
        dma_sync_single_for_cpu(NULL, read_buffer_dma, buffer_size, DMA_FROM_DEVICE);
...
{
///-
    mutex_lock(&pin_share_mutex);
    sunxi_set_nand_pio();
    __nfc_cmdfunc(mtd, command, column, page_addr);
///-
    sunxi_test_nand_pio();
    mutex_unlock(&pin_share_mutex);
}
...
{
///-
	__nfc_cmdfunc(mtd, NAND_CMD_STATUS, -1, -1);
...
///-
	...
	// register IRQ
	if ((err = request_irq(SUNXI_IRQ_NAND, nfc_interrupt_handler, IRQF_DISABLED, "NFC", mtd)) < 0) {
