...
void dma_nand_config_start(int dma, int rw, unsigned int buff_addr, size_t len)
{
    ...
    //set mbus dma mode
    reg_val = readl(NFC_REG_CTL);
    reg_val &= (~(0x1<<15));
    writel(reg_val, NFC_REG_CTL);
    //set dma address & byte counter
    ///-
    //printf("%s %d buff_addr:%x, len:%x\n", __FUNCTION__, __LINE__, addr, len);
    writel(buff_addr, NFC_REG_MDMA_ADDR);
    writel(len, NFC_REG_DMA_CNT);
#endif
}
...
///-
    ...
    while ( (timeout--) && (!(readl(NFC_REG_ST) & NFC_DMA_INT_FLAG)) );
    if (timeout <= 0)
        return -1;
#endif
    return 0;
