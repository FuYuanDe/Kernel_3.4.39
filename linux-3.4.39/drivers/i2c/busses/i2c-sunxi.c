...
static int twi0_proc_open(struct inode *ino, struct file *filp);
static int twi0_proc_close(struct inode *ino, struct file *filp);
static ssize_t twi0_proc_write(struct file *filp, const char __user *buf, size_t count, loff_t *off);
static struct device *twi0_dev = NULL;
static struct file_operations twi0_proc_fops =
{
    .open = twi0_proc_open,
    .release = twi0_proc_close,
    .write = twi0_proc_write,
};
#endif
...
static int __devinit sunxi_i2c_probe(struct platform_device *pdev)
{
	...
	int ret, irq;
	struct proc_dir_entry *proc_res;
	...
	if(0 == i2c->bus_num)
	{
		proc_res = proc_create("twi0_ctl", 0, NULL, &twi0_proc_fops);
		if(!proc_res)
		{
			printk(KERN_ERR "proc_create twi0_proc_fops err!\n");
		}
		else
		{
			twi0_dev = &pdev->dev;
...
static ssize_t twi0_proc_write(struct file *filp, const char __user *buf, size_t count, loff_t *off)
{
	...
	if (0 == strncmp("en", c, 2))
    {
    	...
    	{
#ifdef CONFIG_PM
			sunxi_i2c_resume(twi0_dev);
#endif
    	}
    }
    else if(0 == strncmp("un", c, 2))
    {
    	if(NULL != twi0_dev)
    	{
#ifdef CONFIG_PM
			sunxi_i2c_suspend(twi0_dev);
#endif
    	}
    }
...
static struct i2c_board_info sunxi_i2c_devices[] = {
	{
...
void sunxi_i2c_ds1339(void)
{
	int ret = 0;;
	
	ret = i2c_register_board_info(0, sunxi_i2c_devices, ARRAY_SIZE(sunxi_i2c_devices));
	if (ret < 0) {
		pr_warn("%s()%d - ds1339 init failed!\n", __func__, __LINE__);
	}
	else{
		pr_info("%s()%d - ds1339 init successed!\n", __func__, __LINE__);
	}
}
#endif
