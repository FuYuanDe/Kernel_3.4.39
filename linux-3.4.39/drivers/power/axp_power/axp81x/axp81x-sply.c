...
static int quick_charge_done = 0;
struct delayed_work quick_charge_done_work;
struct delayed_work sply_charge_done_work;
extern uint8_t chg_int_flag;
static int sply_rest_vol = 0;
int pmu_chg_current_status = 0xff;
EXPORT_SYMBOL_GPL(pmu_chg_current_status);
bool pmu_charger_insert_status = false;
EXPORT_SYMBOL_GPL(pmu_charger_insert_status);
...
#endif
	if (pmu_charger_insert_status)
                axp81x_chg_current_limit(axp81x_config.pmu_earlysuspend_chgcur);
        else
                axp81x_chg_current_limit(200000);
        pmu_chg_current_status = 1;
}
static void axp_lateresume(struct early_suspend *h)
{
	...
	if (pmu_charger_insert_status)
                ...
                axp81x_chg_current_limit(200000);
        pmu_chg_current_status = 0;
}
#endif

static void axp_shutdown_led(void)
{
        uint8_t tmp = 0, count = 10;
 
        do {
                axp_update(axp_charger->master, 0x32, 0x00, 0x30);
                axp_read(axp_charger->master, 0x32, &tmp);
        }while((tmp&0x30 != 0x30) && (count-- > 0));

        DBG_PSY_MSG(DEBUG_SPLY, "axp: shutdown LED\n");
}

static void axp_chargdone_quick(struct work_struct *work)
{
    quick_charge_done = 1;
}

static void axp_chargedone_check(struct work_struct *work)
{
        if (sply_rest_vol > 90 && sply_rest_vol < 100)
        {
                axp_charger->rest_vol = ++sply_rest_vol;
                power_supply_changed(&axp_charger->batt);
                schedule_delayed_work(&sply_charge_done_work, msecs_to_jiffies(60 * 1000));
        }
 
        if (100 == sply_rest_vol)
        {
                axp_shutdown_led();
        }
}
...
static void axp_charging_monitor(struct work_struct *work)
{
	...
	uint8_t ocv_percent = 0, columb_percent = 0;
	...
	ocv_percent = (temp_val[0] & 0x7f);
	columb_percent = (temp_val[1] & 0x7f);

	spin_lock(&charger->charger_lock);
	if (1 == quick_charge_done)
		charger->rest_vol = 100;
	else
		charger->rest_vol	= (int)	(val & 0x7F);
	spin_unlock(&charger->charger_lock);

	if (100 == charger->rest_vol)
                axp_shutdown_led();
 
        if (4 == chg_int_flag && charger->rest_vol < 100)
        {
                sply_rest_vol = charger->rest_vol;
                schedule_delayed_work(&sply_charge_done_work, msecs_to_jiffies(60 * 1000));
        }

	if ((1 == charger->is_on) && (0 == quick_charge_done) && (100 == ocv_percent) && (100 == columb_percent) && (99 == charger->rest_vol))
	{
		schedule_delayed_work(&quick_charge_done_work, msecs_to_jiffies(1200 * 1000));
	}else if ((0 == charger->is_on) && (1 == quick_charge_done) && (columb_percent <= 99))
	{
		quick_charge_done = 0;
	}
