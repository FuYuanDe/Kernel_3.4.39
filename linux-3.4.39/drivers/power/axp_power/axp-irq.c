...
EXPORT_SYMBOL_GPL(chg_int_flag);
	 ...
	 void *data)
{
	...
	uint8_t w[11], temp_val;
	int value;

	DBG_PSY_MSG(DEBUG_INT, "axp_battery_event enter...\n");
	axp_reads(charger->master,0x49,1,&temp_val);
        DBG_PSY_MSG(DEBUG_SPLY, "Axp irq test --> 0x49 = %d\n",temp_val);
        chg_int_flag = temp_val;
	if((bool)data==0){
