...
extern int pmu_chg_current_status;
extern bool pmu_charger_insert_status;
...
#ifdef CONFIG_AW_AXP81X
int axp81x_set_chg_current(int current_limit)
{
    int i = 0, tmp_current = 200000;

       while (tmp_current < current_limit) {
           tmp_current += 200000;
               axp81x_chg_current_limit(tmp_current);
               mdelay(200);
       }
       if (tmp_current >= current_limit) {
               tmp_current = current_limit;
               axp81x_chg_current_limit(tmp_current);
       }
	...
	if ((val & 0x02)) { /* usb and ac in short*/
		if((val & 0x10) == 0x00){/*usb or usb adapter can not be used*/
			...
			if((axp_config->pmu_usbcur) && (axp_config->pmu_usbcur_limit)){
				...
				axp_update(axp_charger->master, AXP_CHARGE_CONTROL3, tmp,0xf0);
				msleep(4000);
                                if (pmu_chg_current_status == 0) {
                                        axp81x_set_chg_current(axp_config->pmu_runtime_chgcur);
                                        pmu_charger_insert_status = true;
                                }else if (pmu_chg_current_status == 1) {
                                    axp81x_set_chg_current(axp_config->pmu_earlysuspend_chgcur);
                                        pmu_charger_insert_status = true;
                                }else if (pmu_chg_current_status == 2) {
                                    axp81x_set_chg_current(axp_config->pmu_suspend_chgcur);
                                        pmu_charger_insert_status = true;
                                }else if (pmu_chg_current_status == 3) {
                                    axp81x_set_chg_current(axp_config->pmu_shutdown_chgcur);
                                        pmu_charger_insert_status = true;
                                }else {
                                    axp81x_set_chg_current(200000);
                                }
			}
		}
...
///-
	...
	if (!axp_config->pmu_init_bc_en) {
		...
		do {
                        axp_update(charger->master, 0x32, 0x30, 0x30);
                        axp_read(charger->master, 0x32, &tmp);
                }while((tmp&0x30 != 0x30) && (count-- > 0));
                pmu_charger_insert_status = false;
...
///-
	...
	if (!axp_config->pmu_init_bc_en) {
		...
		do {
                        axp_update(charger->master, 0x32, 0x00, 0x30);
                        axp_read(charger->master, 0x32, &tmp);
                }while((tmp&0x30 != 0x00) && (count-- > 0));
                pmu_charger_insert_status = false;
		if(timer_pending(&charger->usb_status_timer))
			del_timer_sync(&charger->usb_status_timer);
...
///-
	...
	if (axp_config->pmu_init_bc_en) {
		...
		axp_usb_ac_check_status(charger);
		mod_timer(&charger->usb_status_timer,jiffies + msecs_to_jiffies(3000));
