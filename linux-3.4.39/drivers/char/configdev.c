...
static int get_file_length(void)
{
    ...
    /*文件长度私有数据放到区的最后*/
    offset = configdev.total_size - sizeof(length);
...
static int set_file_length(u32 length)
{
    if(sizeof(length) != configdev_write_oneblock(configdev.pfile, (char *)&length, sizeof(length), configdev.total_size-sizeof(length)))
    {
        return -1;
    }
    return 0;
...
///-
    ...
    dataoffset = (*offset);
    if(dataoffset >= (configdev.total_size - sizeof(u32)))
    {
        ret = -ENOSPC;
        goto out0;
    }
    if((dataoffset + count) >= (configdev.total_size - sizeof(u32)))
    {
        count = configdev.total_size - sizeof(u32) - dataoffset;
        old_count = count;
    }
